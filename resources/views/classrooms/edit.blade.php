...
<body id="body-pd" class="bg-white">
    ...
    <div class="bg-white p-5">
        <h4 class="text-center mb-5">Edit Classroom</h4>
        <div class="d-flex justify-content-center align-items-center">
            <form method="post" action="{{ route('updateClassroom', $classrooms->id) }}" class="w-25">
                @csrf
                <div class="mb-3">
                    <label for="grade" class="form-label">Grade</label>
                    <input type="text" name="grade" id="grade" value="{{ $classrooms->grade }}"
                        class="form-control @error('grade')
                            is-invalid
                        @enderror">
                    @error('grade')
                ...
                <div class="mb-3">
                    ...
                    <select name="teacher" id="teacher" class="form-select 
                        @error('teacher')
                            is-invalid
                        @enderror">
                        <option value="{{ $classrooms->teacher_id }}">Select Teacher</option>
